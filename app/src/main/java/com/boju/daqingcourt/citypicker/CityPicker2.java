package com.boju.daqingcourt.citypicker;
...
public class CityPicker2 implements PopupWindow.OnDismissListener, View.OnClickListener {
    ...
    public CityPicker2(Activity context, View parent, boolean isFirst) {
        mContext = context;
        mParent = parent;
        //初始化选择器
    ...
    private void initData(boolean isFirst) {
        mDatas = new Gson().fromJson(Utils.getTextFromAssets(mContext, "city.json"), new TypeToken<List<Province>>() {
        }.getType());
        if(isFirst){
            mProvinceWheel.setData(getProvinceNames());
            mProvinceWheel.setSelect(7);
            mCityWheel.setData(getCityNames(7));
            mCityWheel.setSelect(5);
            mCountyWheel.setData(getCountyNames(7, 5));
            mCountyWheel.setSelect(0);
        }else{
            mProvinceWheel.setData(getProvinceNames());
    ...
    public CityPicker2 setOnCitySelectListener(OnCitySelectListener listener) {
