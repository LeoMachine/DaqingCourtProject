...
public class ArticleRclAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    Context context;
    ...
    List<ArticleInfo> dateAllList;

    public ArticleRclAdapter(Context context, List<ArticleInfo> dateAllList) {
        this.context = context;
        this.dateAllList = dateAllList;
    }

    public final void addItem(int position, ArticleInfo articleInfo) {
        this.dateAllList.add(position, articleInfo);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (mFooterView != null && viewType == TYPE_FOOTER) {
            ...
            View layout = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_article, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (getItemViewType(position) == TYPE_NORMAL) {
            if (holder instanceof ListHolder) {
                ArticleInfo dataInfo;
                if (mHeaderView != null) {
                    dataInfo = dateAllList.get(position - 1);
                } else {
                    dataInfo = dateAllList.get(position);
                }
                GlideUtils.loadImageView(context, dataInfo.getImg_url(), ((ListHolder) holder).iv_image);
                //这里加载数据的时候要注意，是从position-1开始，因为position==0已经被header占用了
                ((ListHolder) holder).tv_title.setText(dataInfo.getTitle());
                ((ListHolder) holder).tv_content.setText(dataInfo.getSummary());
                ((ListHolder) holder).tv_time.setText(dataInfo.getAdd_time());
